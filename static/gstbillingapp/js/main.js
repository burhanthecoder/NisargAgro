...
function initialize_fuse_customers_search_bar() {
  ...
  $(document).bind("focusin click", function (e) {
    if (
      $(e.target).closest("#customer_search_bar, .customer_search_area").length
    )
      return;
...
function initialize_fuse_customers() {
  ...
  $.getJSON("/customersjson", function (data) {
    var fuse_customer_options = {
      ...
      keys: ["customer_name", "customer_address", "customer_gst"],
    };
...
function product_result_to_domstr(result) {
  var domstr =
    "<div class='product-search-result' data-product='" +
    JSON.stringify(result) +
    "'>" +
    "<div>" +
    result["product_name"] +
...
function product_result_click() {
  ...
  selected_item_input
    .parent()
    .parent()
    .find("input[name=invoice-hsn]")
    .val(product_data_json["product_hsn"]);
  selected_item_input
    .parent()
    .parent()
    .find("input[name=invoice-unit]")
    .val(product_data_json["product_unit"]);
  selected_item_input
    .parent()
    .parent()
    .find("input[name=invoice-rate-with-gst]")
    .val(product_data_json["product_rate_with_gst"]);
  selected_item_input
    .parent()
    .parent()
    .find("input[name=invoice-gst-percentage]")
    .val(product_data_json["product_gst_percentage"]);
...
function balance_amount(element) {
  var total_amt_paid = parseFloat(
    element.parent().parent().find("input[name=invoice-paid-amt]").val()
  );
  console.log(total_amt_paid);
  var invoice_total_amt_with_gst = parseFloat(
    $("input[name=invoice-total-amt-with-gst").val()
  );

  console.log(invoice_total_amt_with_gst);
  let balance_left = invoice_total_amt_with_gst - total_amt_paid;
  //   console.log(total_amt_paid, invoice_total_amt_with_gst, balance_left);
  console.log(balance_left);
  element
    .parent()
    .parent()
    .find("input[name=balance-left]")
    .val(parseFloat(balance_left));
}
// START =============================================================
...
$(document).ready(function () {
  ...
  $("input[name=invoice-paid-amt").change(function (e) {
    console.log("lo00l123");
    balance_amount($(this));
  });
